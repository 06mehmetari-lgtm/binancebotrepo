            ...
            try:
                for symbol in self.symbols:
                    async with httpx.AsyncClient(timeout=10.0) as client:
                        resp = await client.get(f"{FUTURES_BASE}/fapi/v1/fundingRate",
                                                params={"symbol": symbol, "limit": 1})
                        data = resp.json()
                    if data:
                        rate = float(data[0]["fundingRate"])
                        await self.redis.set(f"funding:{symbol}", json.dumps({
                            "rate": rate,
    ...
    async def _oi_loop(self):
        while True:
            try:
                for symbol in self.symbols:
                    async with httpx.AsyncClient(timeout=10.0) as client:
                        resp = await client.get(f"{FUTURES_BASE}/fapi/v1/openInterest",
                    ...
                    await self.redis.set(f"oi:{symbol}", json.dumps({
                        "oi": oi, "oi_change_pct": change_pct, "time": time.time()
                    }), ex=300)
            except Exception as e:
            ...
            try:
                for symbol in self.symbols:
                    async with httpx.AsyncClient(timeout=10.0) as client:
                        ...
                        data = resp.json()
                    if data:
                        await self.redis.set(f"ls_ratio:{symbol}", json.dumps({
